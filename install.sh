...
if [ -z "${HICLAW_LANGUAGE}" ]; then
    ...
    _env_file="${HICLAW_ENV_FILE:-$(pwd)/hiclaw-manager.env}"
...
msg() {
    ...
    case "${key}.${lang}" in
        ...
        "install.dir_hint.zh") text="  （env 文件 'hiclaw-manager.env' 将保存到当前目录。）" ;;
        "install.dir_hint.en") text="  (The env file 'hiclaw-manager.env' will be saved to the current directory.)" ;;
...
setup_minio_console_route() {
    ...
    local cookie="/tmp/higress-session-cookie"

    log "Configuring MinIO Console route in Higress (via docker exec)..."

    # Register MinIO Console service source (port 9001)
    ${DOCKER_CMD} exec hiclaw-manager curl -s -o /dev/null -X POST http://127.0.0.1:8001/v1/service-sources \
    ...
    ${DOCKER_CMD} exec hiclaw-manager curl -s -o /dev/null -X POST http://127.0.0.1:8001/v1/domains \
        -b "${cookie}" -H 'Content-Type: application/json' \
        -d '{"name":"'"${minio_console_domain}"'","enableHttps":"off"}' 2>/dev/null || true

    # Create MinIO Console route
    ${DOCKER_CMD} exec hiclaw-manager curl -s -o /dev/null -X POST http://127.0.0.1:8001/v1/routes \
        -b "${cookie}" -H 'Content-Type: application/json' \
...
should_skip_step() {
    ...
    case "${step_fn}" in
        ...
        step_version)
            ...
            ;;
        step_existing)
            local _env="${HICLAW_ENV_FILE:-$(pwd)/hiclaw-manager.env}"
            [ ! -f "${_env}" ] && return 0
            ;;
        step_minio)
            [ "${HICLAW_NON_INTERACTIVE}" = "1" ] && return 0
...
step_existing() {
    local existing_env="${HICLAW_ENV_FILE:-$(pwd)/hiclaw-manager.env}"
...
install_manager() {
    ...
    # Write .env file
    ENV_FILE="${HICLAW_ENV_FILE:-$(pwd)/hiclaw-manager.env}"
    cat > "${ENV_FILE}" << EOF
...
install_worker() {
    ...
    mkdir -p "$(pwd)/hiclaw_data_${WORKER_NAME}"

    # shellcheck disable=SC2086
    ${DOCKER_CMD} run -d \
        --name "${CONTAINER_NAME}" \
        ${DOCKER_ENV} \
        --mount type=bind,source="$(pwd)/hiclaw_data_${WORKER_NAME}",target=/root/hiclaw-fs \
        --restart unless-stopped \
        "${WORKER_IMAGE}"
